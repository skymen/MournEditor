  ...
  export default {
    ...
    methods: {
        ...
        this.$electron.remote.dialog.showOpenDialog(dialogOptions, filePath => {
          if (filePath !== undefined) {
            try {
              ...
              const ace = this.createACEFile()
              const general = this.createGeneralFile()
              // This is not really cool
              let templatePath = this.$electron.remote.app.getAppPath().split('\\')
              templatePath.pop()
      ...
      getPluginFlagsC3 (plugin) {
        const flagsList = plugin.flags
        const replace = this.flags['PluginC3']
        const endArr = []
        flagsList.forEach(flag => {
          endArr.push(replace[flag])
        })
      ...
      getSupportedRuntimes (plugin) {
        const array = plugin.supportedRuntimes
